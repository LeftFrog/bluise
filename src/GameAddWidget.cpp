...
GameAddWidget::GameAddWidget(QWidget* parent) : QWidget(parent)
{
    ...
    working_directory = new GameOptionWidget("Working directory", "", false, this);
...
{
    std::filesystem::copy(choose->path().toStdString(), (bluise_core::DOCS+"res/").toStdString());
    QString header = QFile(choose->path()).fileName();
    bluise_core::games.push_back(Game(name->text(), working_directory->text(), exec->text(), save_path->text(), header));
    close();
}
